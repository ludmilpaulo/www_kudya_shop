...
def upload_proof_of_payment_store(request, order_id):
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"status": "fail", "message": "Error uploading proof of payment"},
                status=400,
            )
...
def upload_proof_of_payment_driver(request, order_id):
    if request.method == "POST":
        try:
            order = Order.objects.get(id=order_id)
            proof_of_payment_file = request.FILES["proof_of_payment"]
            order.proof_of_payment_driver.save(
                proof_of_payment_file.name, proof_of_payment_file
            )
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "message": "Proof of payment to driver uploaded successfully",
                }
            )
        except Exception as e:
            return JsonResponse(
...
def mark_as_paid(request, type, order_id):
    if request.method == "POST":
        try:
            ...
            order.save()
            return JsonResponse(
                {"status": "success", "message": f"Order {type} marked as paid"}
            )
        except Exception as e:
            return JsonResponse(
                {"status": "fail", "message": "Error marking as paid"}, status=400
            )
    return JsonResponse({"status": "fail", "message": "Invalid request"}, status=400)
